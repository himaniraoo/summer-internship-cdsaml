...
if __name__ == "__main__":
    ...
    subjects = np.array([g.split('_')[1] for g in groups])  # adjust your naming if needed
    ...
    print("\n=== LOSO ===")
    for fold, subject in enumerate(unique_subjects):
        test_idx = np.where(subjects == subject)[0]
        train_idx = np.setdiff1d(indices, test_idx)

        train_loader = DataLoader(Subset(dataset, train_idx), batch_size=BATCH_SIZE, shuffle=True)
        val_loader = DataLoader(Subset(dataset, test_idx), batch_size=BATCH_SIZE)
        ...
        for epoch in range(EPOCHS):
            ...
            print(f"Subject {subject} | Epoch {epoch+1} | Loss: {loss:.4f} | Train Acc: {train_acc:.4f} | Val Acc: {val_acc:.4f}")
        ...
        print(f"Subject {subject} | Final Val Acc: {val_acc:.4f} | F1: {fold_f1:.4f}")
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Accuracy")
        plt.title(f"LOSO - Left Out: {subject}")
        plt.legend()
        plt.savefig(f"loso_{subject}_acc_curve.png")
        plt.close()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Mean LOSO Train vs Val Acc")
    plt.legend()
    plt.savefig("mean_loso_acc_curve.png")
    plt.close()

    print(f"\nMean LOSO Acc: {np.mean(fold_accs):.4f} ± {np.std(fold_accs):.4f}")
    print(f"Mean LOSO F1: {np.mean(fold_f1s):.4f} ± {np.std(fold_f1s):.4f}")
    ...
    plt.ylabel("True")
    plt.savefig("pooled_confusion_matrix_loso.png")
    plt.close()
    ...
    }, "final_webcam_gesture_vae_loso.pth")
    print("✅ Final LOSO VAE model saved: final_webcam_gesture_vae_loso.pth")
